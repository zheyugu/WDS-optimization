...
# df = pd.DataFrame(link,index=[0]).T  # transpose to look just like the sheet above
# df.to_excel('link1.xlsx')
...
# set parameter value
D = [0.05, 0.06, 0.08, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40, 0.45] #discrete pipe sizes available [m]
...
CPS = 100 #capital of a pump station $/unit
CP = {0.05:50, 0.06:60, 0.08:80, 0.10:100, 0.15:150, 0.20:200, #material costs of piping 
      0.25:250, 0.30:300, 0.35:350, 0.40:400, 0.45:450}  #  $/m of pipe of diameter k
PS_OM = 1000 #Operations and maintenance cost for pump station [$/PS]
COL_OM = 1000 #Operations and maintenance cost for collection system piping [$/connection]
...
# # do IIS to find problemetic constraints
...
# # Write to csv
# with open('Pipe Size.csv', 'w', newline='') as myfile:
#     wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
#     wr.writerows(zip(var_names, var_values))

# #Write to csv
# varInfo = [(v.varName, v.X) for v in m.getVars()]
# with open('Variable3.csv', 'w', newline='') as myfile:
#     wr = csv.writer(myfile, quoting=csv.QUOTE_ALL)
