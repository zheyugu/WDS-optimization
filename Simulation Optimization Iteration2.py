...
# iteration between simulation and optimization
for iteration in range(10):
    diameter_simulation = wn.query_link_attribute('diameter', np.less, 50)  
    diameter_simulation = diameter_simulation.to_dict()
    
    # take average of last 24 hr flowrates
    flowrate_result = results.link['flowrate']
    flowrate = {}
    m = flowrate_result.index > 774000
    for key,value in dict(zip(zip(x_new, y_new),link_new)).items():
        flowrate [key] = abs(np.mean(flowrate_result.loc[m,[value]].tail(24))[0])
    ...
    # find 6% links with lowest pressure drop
    ...
    Q = flowrate #pipe flow between nodes i and j [m3/s]
    V_min = 0.6 #0.6 [m/s] 
    V_max = 3 #3 [m/s] 
    ...
    reservoir = 'reservoir'
    for i in range(353):
        s.append( [float(s) for s in re.findall(r'-?\d+\.?\d*', var_names[i])])
        if len(s[i]) == 3:
    ...
    diameter_start_end = dict(zip(zip(start, end),diameter))
    
    diameter_optimization = {} 
    for key, value in dict(zip(zip(x_new, y_new),link_new)).items():
        diameter_optimization [value] = diameter_start_end [key]
    
    # assign optimized diameter to simulation model with WNTR
    for key,value in diameter_optimization.items():
    ...
    # count and find the links diameter that is different from last iteration
    link_diff = []
    for key, value in diameter_optimization.items():
        if value != diameter_simulation[key]:
            link_diff.append(key)
    if len(link_diff) == 0:
